...
def clean_and_parse_json(response):
    ...
    json_str = re.sub(r':\s*"([^"]*?)"', lambda m: ': ' +
                      json.dumps(m.group(1)), json_str)
    ...
    try:
        ...
        try:
            ...
            # If it still fails, return the cleaned string for manual inspection
            return f"Failed to parse JSON. Cleaned string: {json_str}"
